        ...
        If ($PSCmdlet.ShouldProcess(("{0}, VMs: {1:N0}" -f $ResourceGroupName,$virtualMachines.Count),("Create Network Monitor Scheduled Task"))) {        
            ...
            ForEach ($vm in $virtualMachines) {
                ...
                If (!(Test-Path -Path ("\\{0}.{1}\admin$\Utilities\NetMon" -f $vm.Name,$domainSuffix))) {
                    ...
                    If (!(Test-Path -Path ("\\{0}.{1}\admin$\Utilities\NetMon\NetMonCapture.xml" -f $vm.Name,$domainSuffix))) {
                        $msg = ("Copying XML to remote computer")
                        Write-Progress @Progress -CurrentOperation $msg
                        Copy-Item -Path ("{0}\NetMonCapture.xml" -f $LocalPath) -Destination ("\\{0}.{1}\admin$\Utilities\NetMon" -f $vm.Name,$domainSuffix) -ErrorAction SilentlyContinue
                    }
                    
                    $SchTask = Invoke-Command -ComputerName ("{0}.{1}" -f $vm.Name,$domainSuffix) -ScriptBlock { & SchTasks.exe /QUERY /TN '\Microsoft\Windows\NetTrace\NetMonCaptures' /FO CSV} -ErrorAction SilentlyContinue | ConvertFrom-Csv
                    If (!$SchTask) {
                        $msg = ("Creating Scheduled Task on remote computer")
                        Write-Progress @Progress -CurrentOperation $msg
                        Invoke-Command -ComputerName ("{0}.{1}" -f $vm.Name,$domainSuffix) -ScriptBlock { & SchTasks.exe /CREATE /TN '\Microsoft\Windows\NetTrace\NetMonCaptures' /XML C:\Windows\Utilities\NetMon\NetMonCapture.xml } | Out-Null
                    }
                    Else {
